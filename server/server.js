...
// import serviceAccountKey from "./full-blog-app-30639-firebase-adminsdk-fbsvc-288423c3a0.json" assert { type: "json" };
...
// Read the Firebase service account JSON directly to avoid import assertion issues
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const serviceAccountPath = path.join(__dirname, 'full-blog-app-30639-firebase-adminsdk-fbsvc-288423c3a0.json');
...
server.post("/update-profile", verifyJWT, (req, res) => {
    ...
    // Ensure social_links is an object (frontend may omit it)
    ...
    try {
        ...
        for(let i = 0; i < socialLinksArr.length; i++){
            ...
            if(link && link.length){
                ...
                if(!hostname.includes(`${key}.com`) && key != 'website'){
                    return res.status(403).json({ error: `${key} link is invalid. You must enter a full link` })
                }
        ...
        return res.status(400).json({ error: "You must provide full social links with http(s) included" })
    }
...
// delete account (user + their blogs/comments/notifications)
server.post("/delete-account", verifyJWT, async (req, res) => {
    ...
    try {
        ...
        // delete notifications created by the user or targeted to the user
        await Notification.deleteMany({ $or: [{ user: user_id }, { notification_for: user_id }] });

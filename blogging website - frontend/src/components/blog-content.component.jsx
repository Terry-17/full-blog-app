...
const Img = ({ url, caption }) => {
    return (
        <div>
            <img src={url} />
            { caption.length ? <p className="w-full text-center my-3 md:mb-12 text-base text-slate-800">{caption}</p> : "" }
        </div>
    )
}
...
const Quote = ({ quote, caption }) => {
    return (
        <div className="bg-yellow-50 p-3 pl-5 border-l-4 border-purple">
            <p className="text-xl leading-10 md:text-2xl">{quote}</p>
            {caption.length ? <p className="w-full text-yellow-400 text-base">{caption}</p> : ""}
        </div>
    )
}
